...
def depth_view(session):
    crzy_resp = session.get(f'http://localhost:{Port}/v1/securities/book?ticker=CRZY_M')
    tame_resp = session.get(f'http://localhost:{Port}/v1/securities/book?ticker=CRZY_A')
